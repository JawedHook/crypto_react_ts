...
const checkNotificationsPerm = async (currentUser: User) => {
  try {
    ...
    if (final !== 'granted') {
      await removePhoneToken(currentUser.id);
      return;
    }
    await setPhoneToken(currentUser.id);
  } catch (err) {
    console.log('checkNotificationsPerm: ', err.message || err.toString());
  }
};
const setPhoneToken = async (currentUserId: string) => {
  try {
...
const removePhoneToken = async (currentUserId: string) => {
  try {
...
const sendNotification = async (currentUser: User, { title, body }) => {
  try {
...
export default { checkNotificationsPerm, setPhoneToken, removePhoneToken, sendNotification };
